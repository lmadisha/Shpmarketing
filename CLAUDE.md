...
- **Root** — Nuxt 4 SPA frontend (Vue 3, `ssr: false`), TypeScript, ESM
- **operations-api/** — Express 5 + Prisma + PostgreSQL backend (CommonJS), port 5001
- **analytics-api/** — small Express + raw `pg` reporting API over a separate analytics database (`performance_reports`, `tenants`), port 5002

The PostgreSQL schema (and DB/user in dev) is named `frostlink`.
...
```bash
npm run dev            # runs operations-api migrations, then frontend + API together (scripts/dev-all.mjs)
...
npx vitest run composables/permissionPolicy.test.ts   # single frontend test file
```

operations-api (cd operations-api):
...
Docker dev stack: `docker compose up` (frontend 5173, API 5001, Postgres 5432; `docker-compose.override.yml` mounts source for hot reload). UAT/prod variants: `docker-compose.uat.yml`, `docker-compose.prod.yml`.

Env setup: copy `.env.example` → `.env` at root and in `operations-api/`. The API exits at startup if `JWT_SECRET` or `MOBILE_API_KEY` is unset.

## Database & Migrations

Two migration systems exist in operations-api — use the reversible one:

- **`operations-api/database/migrations/`** (current) — reversible SQL files with `-- UP` and `-- DOWN` sections, applied by `run-migrations.js` via `npm run migrate:deploy` / `migrate:down`. A post-migrate hook auto-regenerates `database/schema/SCHEMA.md` and `schema.json`; commit migration + updated schema docs together.
- **`operations-api/migrations/`** (legacy) — forward-only SQL applied by `migrate.js` (`npm run migrate`). Don't add new files here.
...
- The full endpoint contract is documented in `operations-api/API_CONTRACT.md` — keep it updated when changing routes.

**analytics-api**: read-only query endpoints using raw SQL over `pool` from `db.js`; no auth layer of its own.
...
- `operations-api/API_CONTRACT.md` — operations-api endpoint contract
- `analytics-api/API_CONTRACT.md` — analytics-api endpoint contract
- `operations-api/database/schema/SCHEMA.md` — generated DB schema reference
