...
async function loadProfile() {
  ...
  try {
    ...
    profileError.value = error instanceof Error ? error.message : 'Could not load profile details.'
  } finally {
...
async function submitProfileUpdate() {
  ...
  try {
    ...
    profileSaveError.value = error instanceof Error ? error.message : 'Could not update profile details.'
  } finally {
...
<template>
  <div class="mx-auto max-w-[1440px] space-y-6 p-4 md:p-6 lg:p-8">
    <div>
      ...
      <p class="mt-1 text-sm text-slate-600">Profile, security, and data validation configuration.</p>
    </div>
      ...
      <div class="space-y-6">
        <Card>
          ...
          <div class="space-y-4 p-5">
            ...
            <p v-if="profileSaveError" class="text-sm text-red-600">{{ profileSaveError }}</p>
            <p v-if="profileSaveSuccess" class="text-sm text-emerald-600">{{ profileSaveSuccess }}</p>
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div class="space-y-1">
                ...
                <Input id="role" :model-value="profile?.permissions || ''" readonly />
              </div>
              <div class="space-y-1">
                <Label for="organisation">Organisation</Label>
                <Input id="organisation" :model-value="profile?.organisation_name || ''" readonly />
              </div>
            </div>
            <div class="flex gap-2">
              <Button :disabled="profileSaving || profileLoading" @click="submitProfileUpdate">{{ profileSaving ? 'Saving...' : 'Save Profile' }}</Button>
              <Button variant="outline" :disabled="profileSaving || profileLoading" @click="loadProfile">{{ profileLoading ? 'Refreshing...' : 'Refresh Profile' }}</Button>
            </div>
            ...
            <div class="border-t border-slate-200 pt-4">
              ...
              <div class="mt-3 space-y-3">
                ...
                <Button :disabled="passwordSaving" @click="submitPasswordChange">{{ passwordSaving ? 'Updating...' : 'Update Password' }}</Button>
              </div>
            </div>
          </div>
        </Card>
