...
async function loadUsers() {
  ...
  try {
    const path =
      isAdmin.value && organisationFilter.value !== "all"
...
const usersByPermission = computed(() => {
  ...
  for (const level of visiblePermissionLevels.value) {
    map.set(
      level,
      filteredUsers.value.filter((u) => u.permissions === level),
    );
  }
  return map;
});
...
async function confirmDeleteOrganisation() {
  ...
  organisationError.value = "";
  try {
    await request(`/organisations/${deletingOrganisation.value.id}`, {
      method: "DELETE",
    });
...
watch(
  [canViewUsers, organisationFilter],
  () => {
    if (canViewUsers.value) {
      void loadUsers();
    }
...
function getLevelIcon(level: string): Component {
  return levelIconMap[level] ?? User;
}
</script>

<template>
  <div class="min-h-full">
    <div v-if="!canViewUsers" class="p-4 md:p-6 lg:p-8">
      <AccessDeniedCard
    ...
    <template v-else>
      ...
      <div
        class="border-b border-slate-200 bg-white px-4 pb-5 pt-6 md:px-6 lg:px-8"
      >
        <div
        ...
        >
          ...
          <div class="flex gap-2">
            ...
            <Button v-if="canManageUsers" @click="openCreateUser()">
              <Plus class="h-4 w-4" />
              Add User
            </Button>
          </div>
        ...
        <div
          :class="`pl-9 grid gap-3 ${isAdmin ? 'md:grid-cols-3' : 'md:grid-cols-2'}`"
        >
        ...
        <p v-if="error" class="mt-3 pl-9 text-sm text-red-600">{{ error }}</p>
      </div>
        ...
        <Card v-if="isAdmin">
          <div class="border-b border-slate-200 p-5">
            <div class="flex items-center gap-2">
              <Building2 class="h-4 w-4 text-[#006aea]" />
              <h2 class="text-lg font-semibold text-slate-900">
                Organisations
              </h2>
            </div>
            <p class="mt-1 text-sm text-slate-600">
              Create or delete organisations.
            </p>
          </div>
          <div class="space-y-4 p-5">
            <div class="grid gap-3 md:grid-cols-4">
              ...
              <div class="space-y-1 md:col-span-2">
                ...
                <Input
                  ...
                  v-model="organisationForm.domin"
              ...
              <div class="flex items-end">
                <Button
                  ...
                  @click="submitCreateOrganisation"
                >
                  <Plus class="h-4 w-4" />
            ...
            <p v-if="organisationError" class="text-sm text-red-600">
              {{ organisationError }}
            </p>
                  ...
                  >
                    ...
                    <td class="px-4 py-3 text-right">
                      <Button
                        ...
                        variant="ghost"
                        class="h-8 w-8"
                        @click="deletingOrganisation = organisation"
            ...
            <div class="flex items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                <div>
                  <div class="flex items-center gap-2">
                    <component
                      :is="getLevelIcon(level)"
                      class="h-4 w-4 text-[#006aea]"
                    />
                    <h2 class="text-lg font-semibold text-slate-900">
                      {{ level }}
                    </h2>
                  ...
                  <p class="mt-1 text-sm text-slate-500">
                    ...
                    {{
                      (usersByPermission.get(level) ?? []).length === 1
                        ? "user"
                        : "users"
                    }}
                  </p>
              ...
              <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                  ...
                  >
                    ...
                    <th v-if="canManageUsers" class="px-4 py-3 text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                ...
                >
                  <tr
                    v-for="user in usersByPermission.get(level)"
                    :key="user.id"
                  >
                    ...
                    <td v-if="isAdmin" class="px-4 py-3 text-slate-600">
                  ...
                  <tr v-if="(usersByPermission.get(level) ?? []).length === 0">
                    <td
                      :colspan="
                        canManageUsers ? (isAdmin ? 6 : 5) : isAdmin ? 5 : 4
                      "
    ...
    <ModalDialog
      ...
      description="Change the assigned role for this user."
    ...
    >
      <div class="space-y-3">
        ...
        <Select
          ...
          "
        />
        <p v-if="permissionError" class="text-sm text-red-600">
          {{ permissionError }}
        </p>
      </div>
      <template #footer>
        <Button variant="outline" @click="permissionTarget = null"
          >Cancel</Button
        >
        <Button :disabled="permissionSaving" @click="submitPermissionChange">{{
    ...
    >
      <p class="text-sm text-slate-600">
        Delete {{ deletingOrganisation?.name }}?
      </p>
      <template #footer>
        <Button variant="outline" @click="deletingOrganisation = null"
          >Cancel</Button
        >
        <Button
          variant="destructive"
