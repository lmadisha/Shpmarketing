...
const organisationId = computed(() => authStore.session?.user.organisation_id ?? null)
...
const bulkErrors = ref<Array<{ rowNumber: number; reason: string; message: string; serial?: string }>>([])
const bulkSkippedRows = ref<BulkSkippedRow[]>([])
const bulkPreviewRows = ref<BulkPreviewRow[]>([])
const bulkPreviewSummary = ref<{ totalRows: number; previewRows: number; excludedRows: number } | null>(null)
...
function validateCreate() {
  ...
  if (!store.actorOrganisationValidationRules) {
    createErrors.value = { serial: 'Could not load organisation validation rules.' }
  ...
  return { isValid: Object.values(createErrors.value).every((value) => !value), serial, mac, cNumber }
}
...
async function submitCreate() {
  ...
  if (!organisationId.value) {
    createResult.value = 'Could not add fridge. Your account has no organisation assigned.'
    return
  }
  ...
  try {
    ...
    createResult.value = 'Could not add fridge. Check duplicates and try again.'
  } finally {
...
async function previewBulkUpload() {
  ...
  try {
    ...
    if (organisationId.value) formData.append('organisation_id', String(organisationId.value))
    const response = await store.adminRequest<{ summary: { totalRows: number; previewRows: number; excludedRows: number }; rows: BulkPreviewRow[] }>('bulkPreview', '/newDevice/bulk/preview', { method: 'POST', body: formData })
    ...
    bulkMessage.value = error instanceof Error ? error.message : 'Bulk preview failed.'
  } finally {
...
async function submitBulkUpload() {
  ...
  try {
    ...
    bulkMessage.value = error instanceof Error ? error.message : 'Bulk upload failed.'
  } finally {
  ...
  <div v-else class="max-w-4xl space-y-6">
    <Card>
      <div class="border-b border-slate-200 p-5">
        ...
        <p class="mt-1 text-sm text-slate-600">Register a new device identity.</p>
      </div>
      <div class="space-y-3 p-5">
        <div class="grid gap-3 md:grid-cols-4">
          <div class="space-y-1">
            <Input :model-value="createForm.fridge_serial_number" placeholder="Serial" @update:model-value="(value) => { createErrors.serial = undefined; createForm.fridge_serial_number = normalizeHexIdentifier(String(value || '')) }" />
            <p v-if="createErrors.serial" class="text-xs text-red-600">{{ createErrors.serial }}</p>
          </div>
          <div class="space-y-1">
            <Input :model-value="createForm.mac_address" placeholder="MAC (optional)" @update:model-value="(value) => { createErrors.mac = undefined; createForm.mac_address = normalizeHexIdentifier(String(value || '')) }" />
            <p v-if="createErrors.mac" class="text-xs text-red-600">{{ createErrors.mac }}</p>
          </div>
          <div class="space-y-1">
            <Input :model-value="createForm.c_number" placeholder="C-Number (optional)" @update:model-value="(value) => { createErrors.cNumber = undefined; createForm.c_number = normalizeCNumber(String(value || '')) }" />
            <p v-if="createErrors.cNumber" class="text-xs text-red-600">{{ createErrors.cNumber }}</p>
          </div>
          <Button :disabled="creating || store.actorOrganisationValidationLoading" @click="submitCreate">{{ creating ? 'Adding...' : 'Add Fridge' }}</Button>
        </div>
        <p v-if="createResult" class="text-sm text-slate-600">{{ createResult }}</p>
      </div>
    </Card>
    ...
    <Card>
      <div class="border-b border-slate-200 p-5">
        ...
        <p class="mt-1 text-sm text-slate-600">Upload CSV or Excel files with serial, optional MAC, and optional C-number columns.</p>
      </div>
      <div class="space-y-4 p-5">
        <div class="flex flex-col gap-3 md:flex-row md:items-center">
          <input type="file" accept=".csv,.xls,.xlsx" class="block w-full rounded-md border border-gray-200 px-3 py-2 text-sm" @change="bulkFile = (($event.target as HTMLInputElement).files || [])[0] || null">
          <Button variant="outline" :disabled="bulkPreviewLoading || !bulkFile" @click="previewBulkUpload">{{ bulkPreviewLoading ? 'Previewing...' : 'Preview File' }}</Button>
          <Button :disabled="bulkSubmitting || !bulkFile" @click="submitBulkUpload">{{ bulkSubmitting ? 'Uploading...' : 'Upload File' }}</Button>
          <Button variant="outline" @click="downloadExcel('fridge_bulk_template.xls', 'Template', ['fridge_serial_number', 'mac_address', 'c_number'], [['A1B2C3D4E5F6', '001122AABBCC', 'C10001']])">
            <Download class="h-4 w-4" />
            Template
          </Button>
        </div>

        <p v-if="bulkMessage" class="text-sm text-slate-600">{{ bulkMessage }}</p>

        <div v-if="bulkErrors.length" class="max-h-40 overflow-auto rounded-md border border-red-200 p-3">
          <p v-for="(item, index) in bulkErrors.slice(0, 20)" :key="`${item.rowNumber}-${index}`" class="text-xs text-red-600">
        ...
        <div v-if="bulkPreviewRows.length" class="max-h-60 overflow-auto rounded-xl border border-slate-200">
          <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="divide-y divide-slate-200 bg-white text-sm text-slate-700">
              <tr v-for="row in bulkPreviewRows.slice(0, 100)" :key="`${row.rowNumber}-${row.fridge_serial_number}`">
    ...
    <ModalDialog :open="skippedModalOpen" title="Skipped Rows Report" description="These rows already exist in the database." max-width-class="max-w-6xl" @close="skippedModalOpen = false">
      <div class="max-h-[420px] overflow-auto rounded-xl border border-slate-200">
        <table class="min-w-full divide-y divide-slate-200">
          ...
          <tbody class="divide-y divide-slate-200 bg-white text-sm text-slate-700">
            <tr v-for="(item, index) in bulkSkippedRows" :key="`${item.rowNumber}-${index}`">
      ...
      <template #footer>
        <Button variant="outline" @click="downloadExcel(`fridge_bulk_skipped_rows_${new Date().toISOString().slice(0, 10)}.xls`, 'Skipped Rows', ['Upload Row', 'Upload Serial', 'Upload MAC Address', 'Upload C-Number', 'Database Serial', 'Database MAC Address', 'Database C-Number', 'Reason', 'Message'], bulkSkippedRows.map((row) => [row.rowNumber, row.serial || '', row.upload_mac_address || '', row.upload_c_number || '', row.db_serial || '', row.db_mac_address || '', row.db_c_number || '', row.reason, row.message]))">
          <Download class="h-4 w-4" />
          Download Report
        </Button>
        <Button variant="outline" @click="skippedModalOpen = false">Close</Button>
      </template>
    </ModalDialog>
  </div>
</template>
